/*
    ...
    2.Edit พิมข้อมูลทับลงบบรทัดเดิมใน total.txt แล้วก้ออกรายงานใหม่
    3.Browsing ถามชื่อ แล้วถามว่าจะเปิดรายงาน หรือ เปิด total.txt
    4.delete ลบข้อมูลของ user คนนั้นในไฟล์ total.txt
*/
#include <stdio.h>
#include <string.h>
#include <math.h>
...
void personal_discount(int *discount);
void insurance(int *discount,int income);
void fund(int *discount,int income);
int netincome(int *discount,int income);
int donate(int netincome);
int rateoftax(int netincome,int otherincome);
int changeschartoint(char number[]);

//report will show on console but total only call on first menu to show on console
int main(void){
    int choice,count = 0,check,income,otherincome,discount = 0,net,don,tax;
    char schoice[255],scheck[255],sincome[255],sotherincome[255];
    char name[100];
    FILE *total,*report;
    ...
    if(choice == 1 || choice == 2){
        ...
        while(check != 1){
            check = 0;
            if(count > 0){
                printf("Error Please fill out again.\n");
                printf("Input your name(Firstname Lastname)\n>> ");
                scanf(" %[^\n]s",name);
            }
        ...
        fprintf(report,"6.deduction (Generall donation)                %10d\n",data[14]);
        ...
        if(choice == 1){
            ...
            */
            ...
            //replace data.
            //หาบรรทัดที่ข้อมูลเก่าอยุ่
            //การทำงานของอันนีคือมันจะถามชื่อแล้วให้ใส่ข้อมูลเพื่อคำนวณใหม่แล้วเอาไปเขียนทับที่เดิม
            char nfptr[] = "text.txt";
            FILE *fptr;
        ...
        printf("Please input your name(Firstname Lastname)\n>> ");
        scanf(" %[^\n]s",name);
        while(check != 1){
            check = 0;
            if(count > 0){
...
void personal_discount(int *discount){
    ...
    int i,number_sons,number_parents,number_disabled,count = 0;
    char snumber_sons[100],snumber_parents[100],snumber_disabled[100],yn;
    do{
    ...
    if(strcmp(status,dict[0]) == 0) /*โสด*/{
        *discount += 60000;
        ...
        }while(number_parents > 2 || number_parents < 0);
        do{
        ...
        }while(number_disabled > 3 || number_disabled < 0);
    }
        ...
        }while(number_disabled > 4 || number_disabled < 0);
    }
...
void insurance(int *discount,int income){
    int Health_insurance,Life_insurance,count = 0,fifteen_percentof_income = (15*income)/100,Annuity_insurance_premium,prakan,interest;
    int *pAnnuity_insurance_premium = &Annuity_insurance_premium;
    do{
        if(count > 0)
            printf("Please fill information again.\n");
        Life_insurance = mistakenumber("Life insurance","maximum 100,000",100000); //เบี้ยประกันชีวิต
        Health_insurance = mistakenumber("Health insuracne","maximum 25,000",25000); //เบี้ยประกันสุขภาพ
        count++;
    }while(Life_insurance + Health_insurance > 100000);
    int Parents_health_insurance = mistakenumber("Parents health insurance premium","maximum 15,000",15000); //เบี้ยประกันสุขภาพบิดา,มารดา
    //system("cls");
    // <= 15% of income and boundaries of annity insurance is 200000 bath. 
    prakan = mistakenumber("Social security","maximum 9,000",9000); //เงินประกันสังคม
    interest = mistakenumber("interest","maximum 100,000",100000); //ดอกเบี้ยที่อยู่อาศัย
...
void fund(int *discount,int income){
    ...
    do{
        if(countt > 0) printf("Please fill out again.\n");
        do{
            if(count > 0) printf("Please fill out again.\n");
        ...
        do{
            if(count > 0) printf("Please fill out again.\n");
            printf("National Savings Fund(less than 13200)\n>> "); //กองทุนออมเงินแห่งชาติ
            scanf("%s",snsf);
...
int donate(int netincome)/*ฟังก์ชันเงินบริจาค*/{
